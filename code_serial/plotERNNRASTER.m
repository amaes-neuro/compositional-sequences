...
for k = 1:neuronNum
    ...
    if (numSpikesNeuron > 0)
        ...
        spksAll(spikeCountTracker+1:spikeCountTracker+numSpikesNeuron,1) = k*ones(numSpikesNeuron,1);
    end
    if (k <= EneuronNum)
        spksExcStruct(k) = struct('times',spikesNeuron'*dt*(10^-3));
    else
        spksInhStruct(k) = struct('times',spikesNeuron'*dt*(10^-3));
    end
